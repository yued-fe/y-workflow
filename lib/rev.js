...
module.exports = (options) => {
  try {
    ...
    throw new Error('要想使用rev功能，请先安装"gulp-rev"、"gulp-rev-delete-original"和"gulp-rev-replace"模块: npm i --save-dev gulp-rev gulp-rev-delete-original gulp-rev-replace');
  }
  ...
  const {
    ...
    taskHandler = () => {
      ...
      return stream.pipe(plugins.rev())
        .pipe(gulp.dest(dest))
